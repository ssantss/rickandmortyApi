...
function play() {
  //  Esta funcion esta enlazada al button que creamos, cada que se le de click o tap va hacer una serie de comparaciones para ir y pintar los sisguientes 20 llamados, que devuelve la api

  if (API === APINEXT) {
    // Si api es igual  a APINEXT  pinta los priemros 20 elementos
    anotherFuncion(API);
  } else if (APINEXT != null) {
    // Si es difernete de null cada que se le da click al boton pinta y pinta los sigueintes 20 hasta que la api llegue a nul y no se pueda hacer nada mas!
